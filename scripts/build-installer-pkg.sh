#!/usr/bin/env bash
# Build macOS release artefacts for the Magenta AU Template host app:
#   - .dmg disk image (drag Magenta AU Template (AU).app to Applications)
...
#
# Usage:
#   ./scripts/build-installer-pkg.sh [--version 0.1.0] [--app path/to/app]
...
APP_BUNDLE_NAME="Magenta AU Template (AU).app"
VOL_NAME="Magenta AU Template"
PKG_ID="com.audiohacking.magenta-au-template"
...
mkdir -p "$OUT_DIR"
PKG_FILE="${OUT_DIR}/Magenta-AU-Template-${PKG_VERSION}-macOS-Installer.pkg"
DMG_FILE="${OUT_DIR}/Magenta-AU-Template-${PKG_VERSION}-macOS.dmg"
...
if [ "$SIGN_APP" = true ]; then
  ...
  APPEX="${APP_PATH}/Contents/PlugIns/MagentaAU_AU.appex"
  if [ -d "$APPEX" ]; then
