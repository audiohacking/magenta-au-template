...
private:
    ...
    /// Set by the inference thread when model state resets; consumed by the
    /// audio thread via drain() so ring cursors are never reset concurrently.
    std::atomic<bool> pending_ring_drain_{false};
};
